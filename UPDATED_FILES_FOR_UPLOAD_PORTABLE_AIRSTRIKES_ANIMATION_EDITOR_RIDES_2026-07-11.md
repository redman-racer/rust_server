# PortableAirstrikes animation editor preview rides and pause fixes - 2026-07-11

What changed:
- `PortableAirstrikesAnimationEditor` is now v0.2.8.
...
- Admins can ride themselves with `/airanim ride` or the preview bar `RIDE` button.
- Admins can put another online player on the ride with `/airanim ride <playerNameOrSteamId>`.
- Riders can be detached and returned to their starting position with `/airanim ride stop [player]` or the preview bar `EXIT` button.
...
- Cargo-plane previews now use the editor's manual waypoint playback path, so `/airanim pause`, the preview bar `PAUSE`, and resume behavior freeze the visible plane instead of only pausing the UI clock.
...
Quick live smoke:
- Confirm the reload banner reports `PortableAirstrikesAnimationEditor` v0.2.8.
...
- Click `RIDE` on the preview bar or run `/airanim ride`; expected result: the admin follows close behind the visible preview vehicle and the editor/preview CUI disappears.
...
- Click `PAUSE` on the preview bar or run `/airanim pause`; expected result: the preview vehicle and preview clock freeze. Click `RESUME` or run `/airanim resume`; expected result: playback continues from the paused timestamp.
- Click `EXIT` or run `/airanim ride stop`; expected result: the admin returns to the position where the ride started.
...
Local verification:
- Roslyn compile check passed for `oxide/plugins/PortableAirstrikesAnimationEditor.cs` v0.2.8 against `RustDedicated_Data/Managed`.
- Remaining warnings were the existing Unity `Rigidbody.velocity` deprecation warnings in preview movement helpers.
- Live Oxide reload and in-game ride/pause smoke testing are still required.
