...
namespace Oxide.Plugins
{
    ...
    public class CopyPaste : CovalencePlugin
    {
        ...
        [HookMethod(nameof(API_RaidlandsTryPaste))]
        public object API_RaidlandsTryPaste(object userId, object filename, object rawArgs)
        {
            ...
            if (string.IsNullOrWhiteSpace(pasteFile))
                return Lang("FILE_NOT_EXISTS", userIdString);

            return TryPasteFromUserId(userIdString, pasteFile, NormalizeRaidlandsPasteArgs(rawArgs));
        }

        [HookMethod(nameof(API_RaidlandsTryPasteAtPosition))]
        public object API_RaidlandsTryPasteAtPosition(object userId, object filename, object rawArgs)
        {
            var userIdString = userId?.ToString();
            var pasteFile = filename?.ToString();
            ...
            var player = players.FindPlayerById(userIdString)?.Object as BasePlayer;
            if (player == null)
                return Lang("NOT_FOUND_PLAYER");

            var args = NormalizeRaidlandsPasteArgs(rawArgs);
            Vector3 pastePosition;
            if (!TryGetRaidlandsPositionOverride(args, out pastePosition))
                return "Raidlands paste position was not supplied";

            return TryPaste(pastePosition, pasteFile, player.IPlayer,
        ...
        {
            return TryPasteFromUserId(userId.ToString(), filename, args ?? new string[0]);
        }

        public object TryPasteFromSteamId(ulong userId, string filename, string[] args, Action callback = null,
            Action<BaseEntity> callbackSpawned = null)
        {
            return TryPasteFromUserId(userId.ToString(), filename, args, callback, callbackSpawned);
        }

        private object TryPasteFromUserId(string userId, string filename, string[] args, Action callback = null,
            Action<BaseEntity> callbackSpawned = null)
        {
            var player = players.FindPlayerById(userId.ToString())?.Object as BasePlayer;
            if (player == null)
        ...
        private string[] NormalizeRaidlandsPasteArgs(object rawArgs)
        {
            if (rawArgs == null)
                return Array.Empty<string>();

            if (rawArgs is string[] stringArray)
                return stringArray;

            if (rawArgs is IEnumerable<string> stringEnumerable)
                return stringEnumerable.ToArray();

            if (rawArgs is object[] objectArray)
                return objectArray.Select(argument => argument?.ToString() ?? string.Empty).ToArray();

            return rawArgs.ToString().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
        }

        private bool TryGetRaidlandsPositionOverride(string[] args, out Vector3 pastePosition)
        {
            pastePosition = Vector3.zero;
            ...
            for (var i = 0; i + 1 < args.Length; i += 2)
            {
                ...
                try
                {
                    pastePosition = args[i + 1].ToVector3();
                    return true;
                }
                catch
                {
        ...
        private void CopyLoop(CopyData copyData)
        {
            ...
            else
            {
                ...
                try
                {
                    Interface.Oxide.DataFileSystem.WriteObject(_subDirectory + copyData.Filename, data);
                }
                catch (Exception exception)
                {
                    var message = $"Copy failed: could not save {_subDirectory}{copyData.Filename}.json ({exception.GetType().Name}: {exception.Message})";
                    copyData.Player.Reply(message);
                    Puts(message);
                    return;
                }
