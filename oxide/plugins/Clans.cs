...
namespace Oxide.Plugins
{
    ...
    public class Clans : CovalencePlugin
    {
        ...
        public bool AcceptAlliance(IPlayer player, string tag)
        {
            ...
            Interface.Oxide.CallHook("OnClanAllianceCreated", clan.Tag, alliedClan.Tag);
        ...
        public bool RevokeAlliance(IPlayer player, string tag)
        {
            ...
            Interface.Oxide.CallHook("OnClanUpdate", clan.Tag);
            Interface.Oxide.CallHook("OnClanUpdate", alliedClan.Tag);
            Interface.Oxide.CallHook("OnClanAllianceDissolved", clan.Tag, alliedClan.Tag);
        ...
        private static readonly DateTime OnClanHookDeprecation = new DateTime(2027, 6, 1);

        #if RUST
        private static readonly List<ulong> RustMemberList = new List<ulong>();
        #endif
        private static readonly List<string> UniversalMemberList = new List<string>();
        
        private static void OnClanMemberJoined(string tag, string joining, ICollection<string> members)
        {
            const string HOOK_NAME = "OnClanMemberJoined";
            const string NEW_HOOK = "OnClanMemberJoined(string tag, string joining, List<string> members)";
            
            #if RUST
            RustMemberList.Clear();
            #endif
            UniversalMemberList.Clear();
            
            foreach (string member in members)
            {
                #if RUST
                RustMemberList.Add(ulong.Parse(member));
                #endif
                UniversalMemberList.Add(member);
            }
            
            #if RUST
            Interface.CallHook(HOOK_NAME, tag, ulong.Parse(joining), RustMemberList);
            #endif
            Interface.CallHook(HOOK_NAME, tag, joining, UniversalMemberList);
            
            // Deprecated
            Interface.Oxide.CallDeprecatedHook(HOOK_NAME, NEW_HOOK, OnClanHookDeprecation, joining, tag);
            Interface.Oxide.CallDeprecatedHook(HOOK_NAME, NEW_HOOK, OnClanHookDeprecation, joining, UniversalMemberList);
        ...
        private static void OnClanMemberGone(string tag, string leaving, ICollection<string> members)
        {
            const string HOOK_NAME = "OnClanMemberGone";
            const string NEW_HOOK = "OnClanMemberGone(string tag, string leaving, List<string> members)";
            ...
            #if RUST
            Interface.CallHook(HOOK_NAME, tag, ulong.Parse(leaving), RustMemberList);
            #endif
            Interface.CallHook(HOOK_NAME, tag, leaving, UniversalMemberList);
            
            // Deprecated
            Interface.Oxide.CallDeprecatedHook(HOOK_NAME, NEW_HOOK, OnClanHookDeprecation, leaving, tag);
            Interface.Oxide.CallDeprecatedHook(HOOK_NAME, NEW_HOOK, OnClanHookDeprecation, leaving, UniversalMemberList);
        ...
        private static void OnClanDisbanded(string tag, ICollection<string> members)
        {
            const string HOOK_NAME = "OnClanDisbanded";
            const string NEW_HOOK = "OnClanDisbanded(string tag, List<string> members)";
            ...
            #if RUST
            Interface.CallHook(HOOK_NAME, tag, RustMemberList);
            #endif
            Interface.CallHook(HOOK_NAME, tag, UniversalMemberList);
            
            // Deprecated
            Interface.CallDeprecatedHook(HOOK_NAME, NEW_HOOK, OnClanHookDeprecation, UniversalMemberList);
            Interface.CallDeprecatedHook(HOOK_NAME, NEW_HOOK, OnClanHookDeprecation, tag);
        ...
        [HookMethod("RaidlandsClanAction")]
        private JObject RaidlandsClanAction(string action, string actorId, string actorName = "", string targetId = "", string targetName = "", string clanTag = "")
        {
            action = RaidlandsNormalizeAction(action);
            actorId = RaidlandsNormalizeSteamId(actorId);
            targetId = RaidlandsNormalizeSteamId(targetId);
            actorName = RaidlandsDisplayName(actorId, actorName);
            targetName = RaidlandsDisplayName(targetId, targetName);

            if (!RaidlandsIsSteamId(actorId))
                return RaidlandsClanResult(false, action, "Invalid actor SteamID64.", clanTag, "", targetId);

            Clan clan = storedData.FindClanByID(actorId);
            if (clan == null)
                return RaidlandsClanResult(false, action, "Actor is not in a clan.", clanTag, "", targetId);

            if (!string.IsNullOrWhiteSpace(clanTag) && !clan.Tag.Equals(clanTag, StringComparison.OrdinalIgnoreCase))
                return RaidlandsClanResult(false, action, "Actor is not a member of the requested clan.", clan.Tag, "", targetId);

            bool isOwner = clan.IsOwner(actorId);
            bool isModerator = clan.IsModerator(actorId);
            string actorRole = isOwner ? "owner" : isModerator ? "moderator" : "member";

            if (!isOwner && !isModerator)
                return RaidlandsClanResult(false, action, "Only clan owners and moderators can manage clans.", clan.Tag, actorRole, targetId);

            switch (action)
            {
                case "invite":
                    return RaidlandsInvitePlayer(clan, actorId, actorName, targetId, targetName, actorRole);

                case "withdraw_invite":
                    return RaidlandsWithdrawInvite(clan, actorName, targetId, targetName, actorRole);

                case "kick":
                    return RaidlandsKickPlayer(clan, actorId, actorName, targetId, actorRole);

                case "promote":
                    if (!isOwner)
                        return RaidlandsClanResult(false, action, "Only the clan owner can promote members.", clan.Tag, actorRole, targetId);

                    return RaidlandsPromotePlayer(clan, actorId, actorName, targetId, actorRole);

                case "demote":
                    if (!isOwner)
                        return RaidlandsClanResult(false, action, "Only the clan owner can demote members.", clan.Tag, actorRole, targetId);

                    return RaidlandsDemotePlayer(clan, actorId, actorName, targetId, actorRole);

                case "disband":
                    if (!isOwner)
                        return RaidlandsClanResult(false, action, "Only the clan owner can disband the clan.", clan.Tag, actorRole, targetId);

                    return RaidlandsDisbandClan(clan, actorRole);
        ...
        private JArray RaidlandsClanSnapshot()
        {
            JArray clans = new JArray();

            foreach (KeyValuePair<string, Clan> entry in storedData.clans)
            {
                Clan clan = entry.Value;
                JObject clanJson = new JObject();
                JArray members = new JArray();
                JArray invites = new JArray();
                JArray allies = new JArray();
                JArray invitedAllies = new JArray();

                clanJson["tag"] = clan.Tag;
                clanJson["description"] = clan.Description ?? "";
                clanJson["owner"] = clan.OwnerID ?? "";
                clanJson["tag_color"] = clan.TagColor ?? "";
                clanJson["created_at_unix"] = clan.CreationTime;
                clanJson["last_online_unix"] = clan.LastOnlineTime;

                foreach (KeyValuePair<string, Clan.Member> member in clan.ClanMembers)
                {
                    JObject memberJson = new JObject();
                    memberJson["steam_id64"] = member.Key;
                    memberJson["display_name"] = member.Value.DisplayName ?? "";
                    memberJson["role"] = member.Value.Role.ToString().ToLowerInvariant();
                    memberJson["is_online"] = member.Value.IsConnected;
                    members.Add(memberJson);
                }

                foreach (KeyValuePair<string, Clan.MemberInvite> invite in clan.MemberInvites)
                {
                    JObject inviteJson = new JObject();
                    inviteJson["steam_id64"] = invite.Key;
                    inviteJson["display_name"] = invite.Value.DisplayName ?? "";
                    inviteJson["created_at_unix"] = invite.Value.ExpiryTime;
                    invites.Add(inviteJson);
                }

                foreach (string ally in clan.Alliances)
                    allies.Add(ally);

                foreach (KeyValuePair<string, double> invitedAlly in clan.AllianceInvites)
                    invitedAllies.Add(invitedAlly.Key);

                clanJson["members"] = members;
                clanJson["member_invites"] = invites;
                clanJson["allies"] = allies;
                clanJson["invited_allies"] = invitedAllies;
                clans.Add(clanJson);
            }
        ...
        private JObject RaidlandsInvitePlayer(Clan clan, string actorId, string actorName, string targetId, string targetName, string actorRole)
        {
            if (!RaidlandsIsSteamId(targetId))
                return RaidlandsClanResult(false, "invite", "Invalid target SteamID64.", clan.Tag, actorRole, targetId);

            if (actorId.Equals(targetId))
                return RaidlandsClanResult(false, "invite", "You cannot invite yourself.", clan.Tag, actorRole, targetId);

            if (storedData.FindClanByID(targetId) != null)
                return RaidlandsClanResult(false, "invite", "Target player is already in a clan.", clan.Tag, actorRole, targetId);

            if (clan.MemberInvites.ContainsKey(targetId))
                return RaidlandsClanResult(false, "invite", "Target player already has a pending clan invite.", clan.Tag, actorRole, targetId);

            if (clan.MemberCount >= configData.Clans.MemberLimit)
                return RaidlandsClanResult(false, "invite", "The clan is already full.", clan.Tag, actorRole, targetId);

            if (clan.MemberInviteCount >= configData.Clans.Invites.MemberInviteLimit)
                return RaidlandsClanResult(false, "invite", "The clan already has the maximum number of invites pending.", clan.Tag, actorRole, targetId);

            IPlayer targetPlayer = covalence.Players.FindPlayerById(targetId);
            ...
            return RaidlandsClanResult(true, "invite", "", clan.Tag, actorRole, targetId);
        }

        private JObject RaidlandsWithdrawInvite(Clan clan, string actorName, string targetId, string targetName, string actorRole)
        {
            ...
            return RaidlandsClanResult(false, "withdraw_invite", "Unable to find a pending invite for that player.", clan.Tag, actorRole, targetId);
        }

        private JObject RaidlandsKickPlayer(Clan clan, string actorId, string actorName, string targetId, string actorRole)
        {
            if (!RaidlandsIsSteamId(targetId))
                return RaidlandsClanResult(false, "kick", "Invalid target SteamID64.", clan.Tag, actorRole, targetId);

            if (actorId.Equals(targetId))
                return RaidlandsClanResult(false, "kick", "You cannot kick yourself.", clan.Tag, actorRole, targetId);

            Clan.Member member;
            if (!clan.ClanMembers.TryGetValue(targetId, out member))
                return RaidlandsClanResult(false, "kick", "Target player is not in your clan.", clan.Tag, actorRole, targetId);

            if (clan.IsOwner(targetId))
                return RaidlandsClanResult(false, "kick", "You cannot kick the clan owner.", clan.Tag, actorRole, targetId);

            if (clan.IsModerator(targetId) && !clan.IsOwner(actorId))
                return RaidlandsClanResult(false, "kick", "Only the clan owner can kick another moderator.", clan.Tag, actorRole, targetId);

            if (member.IsConnected && member.Player != null)
            {
                member.Player.Reply(string.Format(Message("Notification.Kick.PlayerMessage", member.Player.Id), actorName));
                clan.OnPlayerDisconnected(member.Player);
            }

            clan.ClanMembers.Remove(targetId);
            storedData.UnregisterPlayer(targetId);
            clan.Broadcast("Notification.Kick.Reply", actorName, member.DisplayName);
            OnClanMemberGone(clan.Tag, targetId, clan.ClanMembers.Keys);
            Interface.Oxide.CallHook("OnClanUpdate", clan.Tag);
            clan.MarkDirty();

            if (configData.Options.LogChanges)
                LogToFile(Title, $"{member.DisplayName} was kicked from [{clan.Tag}] by {actorName} via Raidlands API", this);

            return RaidlandsClanResult(true, "kick", "", clan.Tag, actorRole, targetId);
        }

        private JObject RaidlandsPromotePlayer(Clan clan, string actorId, string actorName, string targetId, string actorRole)
        {
            if (!RaidlandsIsSteamId(targetId))
                return RaidlandsClanResult(false, "promote", "Invalid target SteamID64.", clan.Tag, actorRole, targetId);

            if (actorId.Equals(targetId))
                return RaidlandsClanResult(false, "promote", "You cannot promote yourself.", clan.Tag, actorRole, targetId);

            Clan.Member member;
            if (!clan.ClanMembers.TryGetValue(targetId, out member))
                return RaidlandsClanResult(false, "promote", "Target player is not in your clan.", clan.Tag, actorRole, targetId);

            if (clan.IsOwner(targetId))
                return RaidlandsClanResult(false, "promote", "You cannot promote the clan owner.", clan.Tag, actorRole, targetId);

            if (clan.IsModerator(targetId))
                return RaidlandsClanResult(false, "promote", "Target player is already a moderator.", clan.Tag, actorRole, targetId);

            if (clan.ModeratorCount >= configData.Clans.ModeratorLimit)
                return RaidlandsClanResult(false, "promote", "The clan already has the maximum number of moderators.", clan.Tag, actorRole, targetId);

            member.Role = Clan.Member.MemberRole.Moderator;
            clan.MarkDirty();
            clan.Broadcast("Notification.Promotion.Reply", member.DisplayName, string.Format(COLORED_LABEL, clan.GetRoleColor(member.Role), member.Role), string.Format(COLORED_LABEL, clan.GetRoleColor(actorId), actorName));
            Interface.Oxide.CallHook("OnClanUpdate", clan.Tag);

            if (configData.Options.LogChanges)
                LogToFile(Title, $"{member.DisplayName} was promoted to {member.Role} by {actorName} via Raidlands API", this);

            return RaidlandsClanResult(true, "promote", "", clan.Tag, actorRole, targetId);
        }

        private JObject RaidlandsDemotePlayer(Clan clan, string actorId, string actorName, string targetId, string actorRole)
        {
            if (!RaidlandsIsSteamId(targetId))
                return RaidlandsClanResult(false, "demote", "Invalid target SteamID64.", clan.Tag, actorRole, targetId);

            if (actorId.Equals(targetId))
                return RaidlandsClanResult(false, "demote", "You cannot demote yourself.", clan.Tag, actorRole, targetId);

            Clan.Member member;
            if (!clan.ClanMembers.TryGetValue(targetId, out member))
                return RaidlandsClanResult(false, "demote", "Target player is not in your clan.", clan.Tag, actorRole, targetId);

            if (clan.IsOwner(targetId))
                return RaidlandsClanResult(false, "demote", "You cannot demote the clan owner.", clan.Tag, actorRole, targetId);

            if (clan.IsMember(targetId))
                return RaidlandsClanResult(false, "demote", "Target player is already a regular member.", clan.Tag, actorRole, targetId);

            member.Role = Clan.Member.MemberRole.Member;
            clan.MarkDirty();
            clan.Broadcast("Notification.Demotion.Reply", member.DisplayName, string.Format(COLORED_LABEL, clan.GetRoleColor(member.Role), member.Role), string.Format(COLORED_LABEL, clan.GetRoleColor(actorId), actorName));
            Interface.Oxide.CallHook("OnClanUpdate", clan.Tag);

            if (configData.Options.LogChanges)
                LogToFile(Title, $"{member.DisplayName} was demoted to {member.Role} by {actorName} via Raidlands API", this);

            return RaidlandsClanResult(true, "demote", "", clan.Tag, actorRole, targetId);
        }

        private JObject RaidlandsDisbandClan(Clan clan, string actorRole)
        {
            string tag = clan.Tag;
            clan.Broadcast("Notification.Disband.Reply", Array.Empty<object>());
            clan.DisbandClan();

            return RaidlandsClanResult(true, "disband", "", tag, actorRole, "");
        }

        private JObject RaidlandsClanResult(bool ok, string action, string error, string clanTag, string actorRole, string targetId)
        {
            JObject result = new JObject();
            result["ok"] = ok;
            result["action"] = action ?? "";
            result["error"] = error ?? "";
            result["clan_tag"] = clanTag ?? "";
            result["actor_role"] = actorRole ?? "";
            result["target_steam_id64"] = targetId ?? "";
            return result;
        }

        private string RaidlandsNormalizeAction(string action)
        {
            action = (action ?? string.Empty).Trim().ToLowerInvariant().Replace("-", "_");

            if (action.Equals("withdraw"))
                return "withdraw_invite";

            return action;
        }

        private string RaidlandsNormalizeSteamId(string steamId)
        {
        ...
        private bool RaidlandsIsSteamId(string steamId)
        {
        ...
        private string RaidlandsDisplayName(string steamId, string displayName)
        {
            ...
            if (RaidlandsIsSteamId(steamId))
            {
                IPlayer player = covalence.Players.FindPlayerById(steamId);
                if (player != null && !string.IsNullOrWhiteSpace(player.Name))
                    return player.Name;
            }

            return string.IsNullOrWhiteSpace(steamId) ? "Unknown player" : steamId;
        }
        #endregion
        ...
        public class Clan
        {
            ...
            internal bool JoinClan(IPlayer player)
            {
                ...
                OnClanMemberJoined(Tag, player.Id, ClanMembers.Keys);
            ...
            internal void DisbandClan()
            {
                ...
                foreach (KeyValuePair<string, Clan> kvp in Instance.storedData.clans)
                    kvp.Value.OnOtherClanDisbanded(Tag);

                OnClanDisbanded(Tag, ClanMembers.Keys);
                OnClanDisbandedInternal();
                
                if (configData.Options.LogChanges)
                    Instance.LogToFile(Instance.Title, $"The clan [{Tag}] was disbanded", Instance);
            }

            internal void OnClanDisbandedInternal()
            {
                ClanMembers.Clear();
                MemberInvites.Clear();
                Alliances.Clear();
                AllianceInvites.Clear();
                IncomingAlliances.Clear();
            }
            
            internal void OnOtherClanDisbanded(string tag)
            {
