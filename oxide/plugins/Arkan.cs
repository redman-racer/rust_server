...
namespace Oxide.Plugins
{
    ...
    class Arkan : RustPlugin
    {
        ...
        #region Helpers
			...
			private string GetBoneName(BasePlayer player, uint boneId)
			{
				if (player?.skeletonProperties == null)
					return string.Empty;

				var boneProperty = player.skeletonProperties.FindBone(boneId);
				if (boneProperty == null)
					return string.Empty;

				string boneName = boneProperty.boneName;
				return string.IsNullOrEmpty(boneName) ? string.Empty : boneName.ToLowerInvariant();
			}
        ...
        private void OnEntityTakeDamage(BasePlayer entity, HitInfo info)
        {
            ...
            {				
                ...
                var _bodyPart = GetBoneName(entity, info.HitBone);
