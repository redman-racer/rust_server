...
{
    [Info("FancyDrop", "FastBurst", "3.2.9")]
    ...
    class FancyDrop : RustPlugin
    {
        ...
        private class UIObject
        {
            ...
            {
                CommunityEntity.ServerInstance.ClientRPC(RpcTarget.Player("AddUI", player), JsonConvert.SerializeObject(ui).Replace("{NEWLINE}", Environment.NewLine));
            }

            public void Destroy(BasePlayer player)
            {
                foreach (string uiName in objectList)
                    CommunityEntity.ServerInstance.ClientRPC(RpcTarget.Player("DestroyUI", player), uiName);
            }
        ...
        private void dropRandom(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args != null && arg.Args.Length > 0)
                if (configData.DropSettings.setupDropTypes.ContainsKey(arg.GetString(0)))
                    type = arg.GetString(0);
                else
                {
            ...
            if (arg.Args != null && arg.Args.Length > 1)
            {
                ...
                foreach (var dtn in configData.StaticItems.LootSettings)
                {
                    switch (dtn.DropTypeName)
                    {
                        ...
                        case "supplysignal":
                        case "custom_event":
                            if (dtn.DropTypeName == arg.GetString(1))
                            {
                                list = arg.GetString(1);
                                if (configData.DebugSettings.useDebug) Puts("List " + arg.GetString(1));
                                exitloop = true;
                            }
                            else
                            {
        ...
        private void dropToPos(ConsoleSystem.Arg arg)
        {
            ...
            string type = "regular";
            if (arg.Args != null && arg.Args.Length > 2)
                if (configData.DropSettings.setupDropTypes.ContainsKey(arg.GetString(2)))
                    type = arg.GetString(2);
                else
                {
                    SendReply(arg, "Droptype not found");
                    return;
            ...
            if (arg.Args != null && arg.Args.Length > 3)
            {
                ...
                foreach (var dtn in configData.StaticItems.LootSettings)
                {
                    switch (dtn.DropTypeName)
                    {
                        ...
                        case "supplysignal":
                        case "custom_event":
                            if (dtn.DropTypeName == arg.GetString(3))
                            {
                                list = arg.GetString(3);
                                if (configData.DebugSettings.useDebug) Puts("List " + arg.GetString(3));
                                exitloop = true;
                            }
                            else
                            {
            ...
            Vector3 newpos = new Vector3(arg.GetInt(0), 0, arg.GetInt(1));
            newpos.y += TerrainMeta.HeightMap.GetHeight(newpos);
            string gridPos = GetGridString(newpos);
        ...
        private void dropMass(ConsoleSystem.Arg arg)
        {
            ...
            {
                int.TryParse(arg.GetString(0), out drops);
                if (drops == 0)
                {
            ...
            if (arg.Args != null && arg.Args.Length > 1)
                if (configData.DropSettings.setupDropTypes.ContainsKey(arg.GetString(1)))
                    type = arg.GetString(1);
                else
                {
            ...
            if (arg.Args != null && arg.Args.Length > 2)
            {
                ...
                foreach (var dtn in configData.StaticItems.LootSettings)
                {
                    switch (dtn.DropTypeName)
                    {
                        ...
                        case "supplysignal":
                        case "custom_event":
                            if (dtn.DropTypeName == arg.GetString(2))
                            {
                                list = arg.GetString(2);
                                if (configData.DebugSettings.useDebug) Puts("List " + arg.GetString(2));
                                exitloop = true;
                            }
                            else
                            {
        ...
        private void dropMassTo(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args.Length >= 3) int.TryParse(arg.GetString(2), out drops);

            if (!float.TryParse(arg.GetString(0), out x) || !float.TryParse(arg.GetString(1), out z))
            {
            ...
            if (arg.Args.Length > 3) float.TryParse(arg.GetString(3), out radius);
            lastDropRadius = radius;
            string type = "massdrop";
            if (arg.Args != null && arg.Args.Length > 4)
                if (configData.DropSettings.setupDropTypes.ContainsKey(arg.GetString(4)))
                    type = arg.GetString(4);
                else
                {
                    SendReply(arg, string.Format("Droptype not found"));
                    return;
            ...
            if (arg.Args != null && arg.Args.Length > 5)
            {
                ...
                foreach (var dtn in configData.StaticItems.LootSettings)
                {
                    switch (dtn.DropTypeName)
                    {
                        ...
                        case "supplysignal":
                        case "custom_event":
                            if (dtn.DropTypeName == arg.GetString(5))
                            {
                                list = arg.GetString(5);
                                if (configData.DebugSettings.useDebug) Puts("List " + arg.GetString(5));
                                exitloop = true;
                            }
                            else
                            {
        ...
        private void dropToPlayer(ConsoleSystem.Arg arg)
        {
            ...
            {
                BasePlayer target = FindPlayerByName(arg.GetString(0));
                if (target == null)
                {
                    SendReply(arg, string.Format($"Player '{arg.GetString(0)}' not found"));
                    return;
                }
                var newpos = new Vector3();
                newpos = target.transform.position;
